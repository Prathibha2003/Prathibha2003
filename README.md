<h1 align="center">Hi 👋, I'm Prathibha A E</h1>

<!-- 🎯 FEATURE 1: Animated Typing Banner -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=1000&color=70A5FD&center=true&vCenter=true&width=600&lines=Full+Stack+Developer+%F0%9F%9A%80;Java+%7C+Python+%7C+JavaScript+%7C+React;Building+Real-World+Applications+%F0%9F%92%BB;Open+to+Full-Time+%26+Internship+Roles+%F0%9F%92%BC" alt="Typing SVG" />
</p>

<p align="center">
  <a href="https://linkedin.com/in/prathibha-a-e">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="/cdn-cgi/l/email-protection#03737162776b6a616b62646c7467623b373343646e626a6f2d606c6e">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://github.com/Prathibha2003">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

<p align="center">
  <a href="https://github.com/Prathibha2003/Prathibha2003/blob/main/Prathibha_Resume.pdf">
    <img src="https://img.shields.io/badge/👁️%20View%20Resume-4CAF50?style=for-the-badge&logoColor=white" />
  </a>
  &nbsp;
  <a href="https://github.com/Prathibha2003/Prathibha2003/raw/main/Prathibha_Resume.pdf">
    <img src="https://img.shields.io/badge/⬇️%20Download%20Resume-2196F3?style=for-the-badge&logoColor=white" />
  </a>
</p>

---

## 🌟 About Me

- 🎓 BE in Computer Science & Engineering — Sri Venkateshwara College of Engineering, Bengaluru (CGPA: 7.94)
- 💻 Software Engineering Trainee at **KodNest**, Bengaluru *(Jun 2025 – Present)*
- 🏭 Former Full Stack Development Intern at **Rail Wheel Factory**, Bengaluru *(Feb 2025 – May 2025)*
- 🧠 Skilled in Java, Python, JavaScript, React.js, Spring Boot, SQL & REST APIs
- 🚀 Passionate about full-stack development & building real-world applications
- 📚 Currently learning: Spring Boot, Advanced React, Microservices & DSA
- 💼 Open to full-time roles, internships & freelance opportunities
- 📍 Based in Bengaluru, India

---

## 🏗️ Tech Stack

### *Languages*
![Java](https://img.shields.io/badge/Java-orange?style=for-the-badge&logo=openjdk)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=for-the-badge&logo=javascript)
![C](https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white)
![HTML](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3)

### *Frameworks & Tools*
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Spring Boot](https://img.shields.io/badge/SpringBoot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white)
![Hibernate](https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=hibernate&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20API-005571?style=for-the-badge&logo=fastapi&logoColor=white)
![JDBC](https://img.shields.io/badge/JDBC-007396?style=for-the-badge&logo=java&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode)

### *Database*
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)

### *Concepts & Practices*
![OOP](https://img.shields.io/badge/OOP-4B8BBE?style=for-the-badge)
![DSA](https://img.shields.io/badge/DSA-FF6F00?style=for-the-badge)
![Agile](https://img.shields.io/badge/Agile-0052CC?style=for-the-badge&logo=jira&logoColor=white)
![SDLC](https://img.shields.io/badge/SDLC-6DB33F?style=for-the-badge)
![IoT](https://img.shields.io/badge/IoT-00B4D8?style=for-the-badge)

---

## 💼 Experience

### 🏭 Full Stack Development Intern — Rail Wheel Factory, Bengaluru
*Feb 2025 – May 2025*
- 🗄️ Engineered an **Employee Database Management System** handling **120+ records**, improving data retrieval efficiency by **30%**
- ⚡ Optimized SQL queries using indexing, reducing execution time by **25%**
- 🐛 Identified and resolved **15+ critical bugs**, enhancing application stability
- 🤝 Collaborated cross-functionally on feature delivery, documentation & project execution
- 🛠️ *Tech: Java, SQL, HTML5, CSS3, JDBC, Git*

### 🎓 Software Engineering Trainee — KodNest, Bengaluru
*Jun 2025 – Present*
- 🌐 Built and tested full-stack web modules using Java, HTML5, CSS3, and SQL with REST API & OOP best practices
- 🧪 Executed functional testing and debugging workflows across multiple training projects
- 📊 Strengthened proficiency in Data Structures, Algorithms & problem-solving
- 🛠️ *Tech: Java, Python, HTML5, CSS3, SQL, Spring Boot*

---

## 🚀 Featured Projects

### 🔹 [TaskPanda – Task Management Web App](https://github.com/Prathibha2003/TaskPanda)
*React.js | JavaScript | HTML5 | CSS3*

A simple and intuitive task manager with filtering, completion tracking, and persistent storage.
- ⚛️ Built with **React.js** using reusable components & state management
- 📦 LocalStorage-based persistent data management
- 🎯 Dynamic task status updates with priority filters
- 📱 Fully responsive single-page application

---

### 🔹 Employee Database Management System
*Java | SQL | JDBC*

A complete database management system for employee records.
- 🗃️ Architected normalized database schema — reduced data redundancy by **20%**
- 🔒 Secure CRUD operations with exception handling
- ⚡ SQL indexing boosted retrieval performance by **30%**

---

### 🔹 Smart Waste Management System
*IoT | Arduino | Sensors | C++*

IoT-based automated waste segregation into metal, wet, and dry categories.
- ⭐ Sponsored by **KSCST – 48th SPP**
- 📱 Real-time monitoring using mobile app
- 📝 Published research paper
- 🤖 Programmed microcontroller logic to process sensor data & trigger automated alerts

---

### 🔹 Face Recognition Attendance System
*Python | OpenCV | Flask*

Automated attendance system using CNN-based face recognition to prevent proxy attendance.
- 👁️ Real-time face detection
- 📋 Attendance auto-logging
- ✅ Reduces manual errors significantly

---

## 🏆 Certifications

| Certificate | Issuer |
|---|---|
| Full Stack Development | KodNest |
| Database and SQL Certification | Infosys Springboard |
| Programming in Python | Meta |
| Introduction to Data Analytics | IBM |

---

## 📊 GitHub Stats & Activity

<!-- 🎯 FEATURE 4: Contribution Streak Counter -->
<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=Prathibha2003&theme=tokyonight&hide_border=true&border_radius=8" alt="GitHub Streak" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Prathibha2003&show_icons=true&theme=tokyonight&hide_border=true&border_radius=8" height="160" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Prathibha2003&layout=compact&theme=tokyonight&hide_border=true&border_radius=8" height="160" />
</p>

<!-- 🎯 FEATURE 3: GitHub Trophies -->
<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Prathibha2003&theme=tokyonight&no-frame=true&no-bg=true&margin-w=6&row=1&column=6" alt="GitHub Trophies" />
</p>

---


<div align="center">
✨ Thanks for visiting my profile!<br/>
💙 Check out my repositories and feel free to connect!
</div>

---

> ### 📌 One-Time Setup Guide *(delete this section after completing)*
>
> **📄 Resume buttons** — Upload your PDF as `Prathibha_Resume.pdf` to the `main` branch of `Prathibha2003/Prathibha2003` repo ✅
